<?php
/**
 * Add support for MediaPress actions to the BoniPress plugin
 */

class PSMT_BoniPress_Actions_Helper extends BoniPress_Hook {
	
	public function __construct ( $hook_prefs, $ptype = 'bonipress_default' ) {
		...
		foreach( $types as $type => $type_object ) {
			...
			$add_media_args['add_' . $type ] = array(
				'creds'        => 0,
				'log'          => '%plural% ' . sprintf( __( 'für neuen %s-Upload', 'psmt-bonipress' ), strtolower( $type_object->singular_name ) ),
				'limit'        => '0/x'
			);
			
			$delete_media_args['delete_' . $type ] = array(
				'creds'        => 0,
				'log'          => '%plural% ' . sprintf( __( 'für Löschen von %s', 'psmt-bonipress' ) , strtolower( $type_object->singular_name ) ),
				'limit'        => '0/x'
			);
	...
	public function run() {
		...
		add_action( 'psmt_media_added',     array( $this, 'new_media' ), 10, 2 );
		add_action( 'psmt_activity_media_marked_attached',     array( $this, 'on_activity_upload' ), 10);
		//try deducting points when a media is deleted
		add_action( 'psmt_delete_media', array( $this, 'delete_media' ) );
	...
	/**
	 * Preferences for MediaPress (See the BoniPress->Hooks screen)
	 */
	public function preferences() {
	
		$prefs = $this->prefs;
		?>
		<?php $types = psmt_get_active_types();?>
		...
		<?php foreach( $types as $type => $type_object ) : 
			...
			$ref = 'upload_' . $type ;
		?>
			<label class="subheader"><?php printf( __( '<strong>%s</strong> Hochladen', 'psmt-bonipress' ), $type_object->singular_name); ?></label>
			<ol>

				<li>
					<label for="<?php echo $this->field_id( array( $key, 'creds' ) ); ?>"><?php  _e( 'Punkte', 'psmt-bonipress' ); ?></label>
				...
				<li>
					<label for="<?php echo $this->field_id( array( $key, 'limit' ) ); ?>"><?php _e( 'Limit', 'psmt-bonipress' ); ?></label>
					<?php echo $this->hook_limit_setting( $this->field_name( array( $key, 'limit' ) ), $this->field_id( array( $key, 'limit' ) ), $prefs[ $key ]['limit'] ); ?>
				</li>

				<li>
					<label for="<?php echo $this->field_id( array( $key, 'log' ) ); ?>"><?php _e( 'Protokollvorlage', 'psmt-bonipress' ); ?></label>
		...
		<?php foreach( $types as $type => $type_object ) : 
			...
			$ref = 'upload_' . $type;
		?>
			<label class="subheader"><?php printf( __( '<strong>%s</strong> löschen', 'psmt-bonipress' ), $type_object->singular_name); ?></label>
			<ol>

					<li>
						<label for="<?php echo $this->field_id( array( $key, 'creds' ) ); ?>"><?php printf( __( '%s löschen ', 'psmt-bonipress' ), $type_object->singular_name ); ?></label>
						<div class="h2"><input type="text" name="<?php echo $this->field_name( array( $key, 'creds' ) ); ?>" id="<?php echo $this->field_id( array( $key, 'creds' ) ); ?>" value="<?php echo $this->core->number( $prefs[ $key ]['creds'] ); ?>" size="8" /></div>
					</li>
